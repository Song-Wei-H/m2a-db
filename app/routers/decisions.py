...
from worker.cve_reassessment import reassess_cve_candidates
...
@router.post(
    "/decisions/reassess-cves/{target_id}",
    response_model=DecisionRunResponse,
)
async def reassess_cves(
    target_id: int,
    db: AsyncSession = Depends(get_db),
) -> DecisionRunResponse:
    """Create a non-executable version-verification decision from CVE candidates."""
    result = await reassess_cve_candidates(db, target_id=target_id)
    return DecisionRunResponse(
        target_id=result.target_id,
        next_action=result.next_action,
        next_tool=result.next_tool,
